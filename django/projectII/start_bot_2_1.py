...
async def get_user_language(user_id: int) -> str:
    res = supabase.table("users").select("language").eq("user_id", user_id).single().execute()
    return res.data.get("language", "ru") if res.data else "ru"
...
TOKEN = "8245152989:AAFcAgLcsojoQFk2siXxuhoo-ZX7DLqEJvA"
ADMIN_ID = 983710534 
...
class AntiFloodMiddleware(BaseMiddleware):
    def __init__(self, time_limit: float = 1.5):
        # Зберігаємо ID юзера на 1.5 секунди. Потім він автоматично видаляється.
    ...
    async def __call__(
        ...
        # Перевіряємо тільки повідомлення та натискання кнопок
        if isinstance(event, (types.Message, types.CallbackQuery)):
            user_id = event.from_user.id
            if user_id in self.limit:
                # Якщо юзер спамить - ігноруємо запит мовчки
                return
            else:
                # Записуємо юзера в кеш
...
def get_sub_keyboard(lang: str):
    ...
    builder.row(types.InlineKeyboardButton(text="💰 ISH | о крипте", callback_data="go_link_2"))
    builder.row(types.InlineKeyboardButton(text="💻 Чатыч", callback_data="go_link_3")) 
...
async def track_and_redirect(callback: types.CallbackQuery, state: FSMContext):
    ...
    urls = {
        "link_2": "https://t.me/+NuiQ2jBMf5g1MDEy",
        "link_3": "https://t.me/gegegepetezloy_bot?start=utm_Wghwnshsian010"
    }
...
async def handle_text(message: types.Message, state: FSMContext):
    ...
    food_keywords = ["з'їв", "з'їла", "съел", "съела", "ate", "жедім", "калорії", "калории", "стакан", "порція", "тарілка", "грам"]
    is_food_log = any(word in text.lower() for word in food_keywords)
    ...
    if is_food_log:
        try:
            ...
            prompt = f"""You are an automated API. User text: "{text}". 
            Extract nutritional data. Output ONLY a raw JSON object. No markdown, no text.
            Format EXACTLY: {{"calories": 150, "proteins": 8, "fats": 5, "carbs": 12, "food_name": "Name in {target_lang}"}}"""
            
            response = await asyncio.wait_for(model.generate_content_async(prompt), timeout=25.0)
            full_text = response.text.strip()
            
            start = full_text.find('{')
            end = full_text.rfind('}')
            if start != -1 and end != -1 and end > start:
                nutrients = json.loads(full_text[start:end+1])
                
                supabase.table("nutrition_logs").insert({
                    "user_id": user_id, "calories": int(nutrients.get("calories", 0)),
                    "proteins": int(nutrients.get("proteins", 0)), "fats": int(nutrients.get("fats", 0)),
                    "carbs": int(nutrients.get("carbs", 0)), "food_name": str(nutrients.get("food_name", "Food"))
                }).execute()
                ...
                final_reply = f"✅ <b>{nutrients.get('food_name')}</b> {t['added']}\n\n🔥 {t['cal']}: {nutrients.get('calories')} kcal\n🥩 {t['prot']}: {nutrients.get('proteins')} g\n🧈 {t['fat']}: {nutrients.get('fats')} g\n🍞 {t['carb']}: {nutrients.get('carbs')} g"
                await msg.edit_text(final_reply, parse_mode="HTML")
            else:
                await msg.edit_text(f"❌ {t['err']}")
        ...
        try:
            ...
            final_prompt = f"Instruction: Answer strictly in {target_lang}. User info: {user.get('age')}y.o, {user.get('weight')}kg. Request: {query_text}"
            
            response = await asyncio.wait_for(model.generate_content_async(final_prompt), timeout=30.0)
            ...
            await msg.edit_text("⏳ AI is overloaded. Try again.")
        except Exception as e:
...
async def analyze_food_with_text(message: types.Message):
    ...
    try:
        ...
        food_image = genai.upload_file(path=path)
        
        prompt = [
            food_image,
            f"You are an API that ONLY outputs JSON. User text: {user_text}. Context: {ctx}\n"
            f"Analyze this meal and estimate calories, proteins, fats, and carbs.\n"
            f"CRITICAL RULE: Output ONLY a valid JSON object. No markdown, no explanations.\n"
            f"Format EXACTLY like this:\n"
            f"{{\"calories\": 100, \"proteins\": 10, \"fats\": 5, \"carbs\": 20, \"food_name\": \"Name in {target_lang}\"}}"
        ]
        ...
        # ЦЕЙ БЛОК ВРЯТУЄ ТВІЙ СЕРВЕР ВІД ПЕРЕПОВНЕННЯ
        if os.path.exists(path):
            os.remove(path)
